...
2. 运行 Yong 输入法进入设置程序，授予有关权限，包括：悬浮窗权限（必须的）、所有文件管理权限（必须的）、访问媒体文件、接收短信、麦克风权限、读取联系人、摄像头、读取剪贴板等。

3. 在手机系统设置的“管理输入法”（可通过 **Yong 输入法设置程序 🞂 安卓 🞂 设置系统输入法**快速打开）中，启用 Yong 输入法键盘。

4. 在任意文本框呼出当前的输入法键盘，并切换至 Yong 输入法键盘（如果当前输入法不提供切换功能，就在手机系统设置的“当前输入法”中切换），这时大概率不会弹出 Yong 输入法键盘，是正常的。<br>这步操作目的是在手机的 /sdcard/yong/.yong 隐藏目录下自动生成 yong.ini，作为替代手段，你也可以手动把 /sdcard/yong/yong.ini 复制为 /sdcard/yong/.yong/yong.ini。
...
升级方式一（升级程序和码表）：手动下载[小小星空输入法 Android 版安装程序][小小星空网盘]并覆盖安装。（注意：如果安装包的版本与已安装的相同，将不会覆盖已经存在的码表文件）
