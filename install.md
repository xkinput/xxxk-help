...
4. 下载[:fa fa-github: 本项目源代码][小小星空仓库]，将 home 目录里的内容（最关键的是 mb 目录和 entry 目录）复制到配置目录 ~/.yong（这是个隐藏目录），再把 yong-win.ini 重命名为 yong.ini（替换掉原本的 yong.ini）。
...
前者可以通过**右击输入法状态条 🞂 小小输入法程序（ooxx） 🞂 更新程序文件**来自动从小小输入法的服务器获取更新（也可以手动[安装](#安装)新版程序）。

后者目前没有自动升级方式，只能手动下载本项目的文件来进行升级。方法是：[退出](#退出)小小输入法，然后执行[安装](#安装)的第 4 步。
...
2. 在 Yong 输入法设置程序中进行个性化设置（例如在“输入法”页选择默认输入方案，在“安卓”页开启有关功能；注意每个页面修改后要点右上角的保存才会生效）。
