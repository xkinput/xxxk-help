...
安装到设备上的小小星空主要由两部分组成：小小输入法程序 + 星空方案码表。

升级方式一（升级程序和码表）：手动下载并运行最新的[小小星空输入法 Windows 版安装程序][小小星空网盘]。

升级方式二（只升级程序）：**右击输入法状态条 🞂 更新 🞂 更新程序文件**，可自动从小小输入法的服务器获取程序更新。

升级方式三（只升级键道方案码表）：**右击输入法状态条 🞂 更新 🞂 更新键道码表**，可自动将键道码表同步至 [RIME键道仓库] 最新状态。

> [!TIP|label:鼓励用户维护自己的码表]
> 1. 官方码表仅供参考，我们更鼓励用户定制并维护自己的码表。
> 2. 用户可以考虑使用[小小输入法为提供的免费同步空间](https://yong.dgod.net/read.php?tid=651)来同步个人码表和设置。
...
> 由于 Win10 的一个[疑似 bug](https://answers.microsoft.com/zh-hans/windows/forum/all/%E7%A8%8B%E5%BA%8F%E7%9A%84%E5%8D%B8%E8%BD%BD/0b57356b-e988-486e-b863-1cf63e0cf0d2)，在开始菜单中可能无法显示卸载程序，你可以改为运行程序目录下的 uninstall.exe。
...
方式一（自动安装）：直接使用[小小星空输入法 Android 版安装程序][小小星空网盘]。

方式二（手动安装）：

1. 安装[原版小小输入法 Android 版][小小输入法网盘]。
2. 下载[:fa fa-github: 本项目源代码][小小星空仓库]，解压。
3. 将仓库 home/yong-android.ini 更名为 yong.ini，复制并覆盖掉 /sdcard/yong/yong.ini 和 /sdcard/yong/.yong/yong.ini。
4. 将仓库 home/version.txt 复制并覆盖掉 /sdcard/yong/version.txt。
5. 将仓库 home/skin-android 里的内容复制到 /sdcard/yong/.yong/android 目录中。
6. 将仓库 home 目录里的其他内容（最关键的是 mb 目录）复制到 /sdcard/yong/.yong 隐藏目录中。

### 启动

如果你是初次安装，请执行以下步骤：

1. 运行 Yong 输入法进入设置程序，授予有关权限，包括：悬浮窗权限（必须的）、所有文件管理权限（必须的）、访问媒体文件、接收短信、麦克风权限、读取联系人、摄像头、读取剪贴板等。

2. 在手机系统设置的“管理输入法”（可通过 **Yong 输入法设置程序 🞂 安卓 🞂 设置系统输入法**快速打开）中，启用 Yong 输入法键盘。

3. 在任意文本框呼出当前的输入法键盘，并切换至 Yong 输入法键盘（如果当前输入法不提供切换功能，就在手机系统设置的“当前输入法”中切换），这时大概率不会弹出 Yong 输入法键盘，是正常的。<br>这步操作目的是在手机的 /sdcard/yong/.yong 隐藏目录下自动生成 yong.ini，作为替代手段，你也可以手动把 /sdcard/yong/yong.ini 复制为 /sdcard/yong/.yong/yong.ini。
...
![管理输入法](_media\settings-current-ime-android.webp '当前输入法和管理输入法')
...
升级方式一（升级程序和码表）：手动下载[小小星空输入法 Android 版安装程序][小小星空网盘]并覆盖安装。

升级方式二（只升级程序）：在 **Yong 输入法设置程序 🞂 服务 🞂 更新**中下载最新的原版小小输入法安装包并覆盖安装。
